...
def load_augmentation_with_compat(aug_path, logger=None):
    """Load augmentation with compatibility for JpegCompression -> ImageCompression"""
    if aug_path is None:
        return None
    
    # Read the yaml file and replace JpegCompression with ImageCompression
    with open(aug_path, 'r') as f:
        content = f.read()
    
    # Replace deprecated transform name
    if 'JpegCompression' in content:
        content = content.replace('JpegCompression', 'ImageCompression')
        if logger:
            logger.info('Replaced JpegCompression with ImageCompression for compatibility')
        else:
            print('Replaced JpegCompression with ImageCompression for compatibility')
    ...
    try:
        aug = albumentations.load(tmp_path, data_format='yaml')
    finally:
        os.remove(tmp_path)
...
def main():
    ...
    aug_train = load_augmentation_with_compat(config.TRAIN.AUG, logger)
    aug_valid = load_augmentation_with_compat(config.VALID.AUG, logger)
