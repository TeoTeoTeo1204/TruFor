...
class FullModel(nn.Module):
    ...
    def __init__(self, model, config=None):
        ...
        # Lấy các loss đã gửi sang device phù hợp
        self.loss_loc, self.loss_conf, self.loss_det = get_criterion(config)

    def forward(self, labels=None, rgbs=None):
        outputs, conf, det, npp = self.model(rgbs.cuda())
        final_loss = 0
        for (l, w, _) in self.losses:
            if l == 'LOC':
                loss = self.loss_loc(outputs, labels.cuda())
            elif l == 'CONF':
                loss = self.loss_conf(outputs, labels.cuda(), conf)
            elif l == 'DET':
                loss = self.loss_det(det, labels.cuda())
